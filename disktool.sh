...
echo "Script is starting on $(date)"
...
run_smartctl_a() {
    ...
    process_and_store_output() {
        ...
        # Skip storing if modified output contains "INQUIRY failed"
        if echo "$MODIFIED_OUTPUT" | grep -q "INQUIRY failed"; then
            # echo "-------------------------------------------------------------------------------------------------------------------------------------------"
            echo "Skipping output: INQUIRY failed detected"
            # echo "DEBUG_DATA:host:$(hostname),disk_path:$DEVICE,mount_path:$MOUNT_PATH|||$MODIFIED_OUTPUT"
            # echo "-------------------------------------------------------------------------------------------------------------------------------------------"
            return
        fi

        # Skip storing only if both "Vendor:" and "Product:" have values in modified output
        if echo "$MODIFIED_OUTPUT" | grep -qE "Vendor:\s*\S+" && echo "$MODIFIED_OUTPUT" | grep -qE "Product:\s*\S+"; then
            # echo "-------------------------------------------------------------------------------------------------------------------------------------------"
            echo "Skipping output: Both Vendor and Product information detected"
            # echo "DEBUG_DATA:host:$(hostname),disk_path:$DEVICE,mount_path:$MOUNT_PATH|||$MODIFIED_OUTPUT"
            # echo "-------------------------------------------------------------------------------------------------------------------------------------------"
            return
        fi

        # Skip storing specific cciss failure message
        if echo "$MODIFIED_OUTPUT" | grep -qE "Smartctl open device: .* \[cciss_disk_[0-9]{2}\] \[SCSI/SAT\] failed: INQUIRY \[SAT\]: No such device or address"; then
            # echo "-------------------------------------------------------------------------------------------------------------------------------------------"
            echo "Skipping output: Specific cciss failure detected"
            # echo "DEBUG_DATA:host:$(hostname),disk_path:$DEVICE,mount_path:$MOUNT_PATH|||$MODIFIED_OUTPUT"
            # echo "-------------------------------------------------------------------------------------------------------------------------------------------"
            return
        fi
        ...
        # echo "DISK_HEALTH_DATA:host:$(hostname),disk_path:$DEVICE,mount_path:$MOUNT_PATH|||$MODIFIED_OUTPUT"
    ...
    if echo "$OUTPUT" | grep -q "please try adding '-d megaraid"; then
        ...
        while :; do
            ...
            process_and_store_output "$OUTPUT" "$DEVICE" "$MOUNT_PATH"
            MEGARAID_ID=$((MEGARAID_ID + 1))
        done
    fi
    ...
    # Process and store the final output
    process_and_store_output "$OUTPUT" "$DEVICE" "$MOUNT_PATH"
